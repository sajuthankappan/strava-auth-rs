...
async fn test_create_access_token() {
    ...
    let access_token = api_client.token_api.create_token(authorization_code).await.unwrap();
    dbg!(access_token);
}
...
async fn test_refresh_access_token() {
    ...
    let configuration = Configuration::new(client_id, client_secret);
    let api_client = AuthClient::new(configuration);
    let access_token = api_client.token_api.refresh_token(refresh_token).await.unwrap();
    dbg!(access_token);
}